...
- (void)setProgress:(float)newProgress {
    ...
    //，
    if (mode == ActivityModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(RoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == ActivityModeDeterminate) {
        self.indicator = [[[RoundProgressView alloc] init] autorelease];
    }
    else if (mode == ActivityModeCustomView && self.customView != nil){
...
+ (ActivityIndicator *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	ActivityIndicator *hud = [[ActivityIndicator alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[ActivityIndicator class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		ActivityIndicator *HUD = (ActivityIndicator *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"ActivityViewIsNillException" 
					format:@"The view used in the Activity initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = ActivityAnimationFade;
        self.mode = ActivityModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == ActivityAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == ActivityAnimationZoom) {
